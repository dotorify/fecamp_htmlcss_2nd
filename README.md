...
- 브라우저 지원은 어디까지 고려해야할까?
  ```
  1. 정책을 세울 때 정해야 한다.
  2. IE버전을 몇 까지 지원할지 고민해야 한다.
    => choeun's: 최신버전 -2 단계까지 지원 (IE 9~10부터 지원)
  3. 서로다른 브라우저에서 최대한 동일하게 보이도록 하려면
...
- 디버깅
  ```
  1. 크롬 최신버전, Mac OS, 내 사이트의 사이즈와 브라우저 사이즈를 비교했을 때, 어떤 상황에서 장애가 나는가
  2. 내 사이트가 width가 900px, 브라우저 크기가 그 보다 작아요. 근데 xx가 안되요. 왜 안될까?
  ```
